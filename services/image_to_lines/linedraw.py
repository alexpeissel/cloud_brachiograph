...
# Generates JSON vector representation of input image.
def generate_vector_json(data):
    file_data = data
    ...
    _, temp_local_output_filename = tempfile.mkstemp()
    _, temp_local_svg_filename = tempfile.mkstemp()
    ...
    processed_image_bucket = storage_client.bucket(processed_image_bucket_name)

    new_json_blob = processed_image_bucket.blob(f'{file_name}.json')
    new_json_blob.upload_from_filename(temp_local_output_filename)
    print(f'Vector JSON uploaded to: gs://{processed_image_bucket_name}/{file_name}.json')

    new_svg_blob = processed_image_bucket.blob(f'{file_name}.svg')
    new_svg_blob.upload_from_filename(temp_local_svg_filename)
    print(f'SVG uploaded to: gs://{processed_image_bucket_name}/{file_name}.svg')

    # Delete the temporary files.
    os.remove(temp_local_filename)
    os.remove(temp_local_output_filename)
    os.remove(temp_local_svg_filename)
...
def vectorise(
    ...
    if draw_contours and repeat_contours:
        contours = sortlines(getcontours(
            image.resize((int(resolution/draw_contours),
    ...
    if draw_hatch and repeat_hatch:
        hatches = sortlines(
            hatch(
                # image,
                image.resize((int(resolution/draw_hatch),
                              int(resolution/draw_hatch*h/w))),
                draw_hatch
            ))
...
def getcontours(image, draw_contours=2):
    ...
    for i in range(0, len(contours)):
        contours[i] = [(v[0]*draw_contours, v[1]*draw_contours)
                       for v in contours[i]]
    ...
    for x0 in range(w):
        ...
        for y0 in range(h):
            ...
            # below 16, draw diagonal lines and a second horizontal line
                ...
                lg1.append([(x, y+draw_hatch/2+draw_hatch/4),
...
F_SobelX = {(-1, -1): 1, (0, -1): 0, (1, -1): -1, (-1, 0): 2,
            (0, 0): 0, (1, 0): -2, (-1, 1): 1, (0, 1): 0, (1, 1): -1}
